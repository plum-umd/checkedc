...
Inductive eval_type_base (s:stack) : type -> type -> Prop :=
   ...
   | cast_type_base_ptr : forall c pm t t', eval_type_base s (TPtr c pm t) (TPtr c (eval_base s pm) t')
...
Inductive step (D : structdef) (U: union) (F:Z -> option (list (var * type) * type * expression * mode)) : stack -> real_heap 
  ...
  | StrlenChecked : forall s H1 H2 x pm n n' l h t t1, 
     ...
     (forall i , n <= i < n+n'+1 -> (exists n1, Heap.MapsTo i (n1,t1) H1 /\ n1 <> 0))
      -> Heap.MapsTo (n+n'+1) (0,t1) H1 ->
            step D U F s (H1,H2) (EStrlen x) (change_strlen_stack s x Checked pm t l n n' h) (H1,H2) (RExpr (ELit n' TNat))

  | StrlenTainted : forall s H1 H2 x pm n n' l h t t1, 
     h > n -> l <= n -> 0 <= n' -> pm <> None ->
     (Stack.MapsTo x (n,(TPtr Tainted pm (TNTArray (Num l) (Num h) t))) s) ->
     (forall i , n <= i < n+n'+1 -> (exists n1, Heap.MapsTo i (n1,t1) H2 /\ n1 <> 0 
              /\ well_typed_lit D U empty_theta H2 empty_scope n1 t1))
      -> Heap.MapsTo (n+n'+1) (0,t1) H2 ->
            step D U F s (H1,H2) (EStrlen x) (change_strlen_stack s x Tainted pm t l n n' h) (H1,H2) (RExpr (ELit n' TNat))
  | StrlenUnChecked : forall s H1 H2 x pm n n' t t1, 
     0 <= n' ->
     (Stack.MapsTo x (n,(TPtr Unchecked pm t)) s) ->
     (forall i , n <= i < n+n'+1 -> (exists n1, Heap.MapsTo i (n1,t1) H2 /\ n1 <> 0))
      -> Heap.MapsTo (n+n'+1) (0,t1) H2 ->
            step D U F s (H1,H2) (EStrlen x) s (H1,H2) (RExpr (ELit n' TNat))
  | StrlenNone : forall s H1 H2 x m  n l h t, m <> Unchecked ->
     (Stack.MapsTo x (n,(TPtr m None (TNTArray l h t))) s) ->
  ...
  | StrlenLowOOB : forall s H x m pm n t l h,
     ...
     (Stack.MapsTo x (n,(TPtr m pm (TNTArray (Num l) h t))) s) ->
      step D U F s H (EStrlen x) s H RBounds
  | StrlenNull : forall s H x t m pm n l h,
      n <= 0 -> m <> Unchecked ->
  ...
  | SLet : forall s H x n t e t',
      ...
      step D U F s H (ELet x (ELit n t) e) (Stack.add x (n,t') s) H 
                     (RExpr (ERet x (n,t') (Stack.find x s) e))

  | SRetSome : forall s H x a ta ntb n t, 
          step D U F s H (ERet x ntb (Some (a,ta)) (ELit n t))
                  (Stack.add x (a,ta) s) H (RExpr (ELit n t))
  | SRetNone : forall s H x ntb n t, 
          step D U F s H (ERet x ntb None (ELit n t))
  ...
  | SPlusNull : forall s H n1 t n2,
      ...
      step D U F s H (EPlus (ELit n1 t) (ELit n2 (TNat))) s H RNull
  | SCast : forall s H t n t' t'' ta,
      eval_type_bound s t t'' ->
      eval_type_base s t'' ta ->
      step D U F
         s H (ECast t (ELit n t'))
         s H (RExpr (ELit n ta))

  | SCastNoArray : forall s H x y t n m pm pm' t' t'' ta,
     ~ is_array_ptr (TPtr m pm t') -> eval_type_bound s t t'' ->
       eval_type_base s t'' ta ->
      step D U F
        s H (EDynCast (TPtr m pm' (TArray x y t)) (ELit n (TPtr m pm t')))
        s H (RExpr (ELit n (TPtr m pm' (TArray (Num n) (Num (n+1)) ta))))

  | SCastArray : forall s H t n pm t' l h w l' h' w',
     eval_type_bound s t (TPtr Checked pm (TArray (Num l) (Num h) w)) ->
      eval_type_bound s t' (TPtr Checked pm (TArray (Num l') (Num h') w')) ->
          l' <= l -> l < h -> h <= h' ->
      step D U F
         s H (EDynCast t (ELit n t'))
         s H (RExpr (ELit n (TPtr Checked pm (TArray (Num l) (Num h) w))))
  ...
  | SCastArrayHighOOB1 : forall s H t n pm t' l h w l' h' w',
     eval_type_bound s t (TPtr Checked pm (TArray (Num l) (Num h) w)) ->
      eval_type_bound s t' (TPtr Checked pm (TArray (Num l') (Num h') w')) ->
           h' < h -> 
           step D U F s H (EDynCast t (ELit n t')) s H RBounds
  | SCastNTArray : forall s H t n pm t' l h w l' h' w',
     eval_type_bound s t (TPtr Checked pm (TNTArray (Num l) (Num h) w)) ->
      eval_type_bound s t' (TPtr Checked pm (TNTArray (Num l') (Num h') w')) ->
          l' <= l -> l < h -> h <= h' ->
      step D U F s H (EDynCast t (ELit n t')) s H (RExpr (ELit n (TPtr Checked pm (TNTArray (Num l) (Num h) w)) ))
  ...
  | SCastNTArrayHighOOB1 : forall s H t n pm t' l h w l' h' w',
     eval_type_bound s t (TPtr Checked pm (TNTArray (Num l) (Num h) w)) ->
      eval_type_bound s t' (TPtr Checked pm (TNTArray (Num l') (Num h') w')) ->
           h' < h -> 
           step D U F s H (EDynCast t (ELit n t')) s H RBounds
  | SDerefChecked : forall s H1 H2 n pm n1 t1 t t2 tv,
      eval_type_bound s (TPtr Checked pm t) t2 -> pm <> None ->
      Heap.MapsTo n (n1, t1) H1 ->
      (forall l h t', t2 = TPtr Checked pm (TArray (Num l) (Num h) t') -> l <= n < h) ->
      (forall l h t', t2 = TPtr Checked pm (TNTArray (Num l) (Num h) t') -> l <= n < h) ->
      @get_root D t2 tv ->
      step D U F s (H1,H2) (EDeref (ELit n (TPtr Checked pm t))) s (H1,H2) (RExpr (ELit n1 tv))
  | SDerefTainted : forall s H1 H2 n pm n1 t1 t t2 tv,
      eval_type_bound s (TPtr Tainted pm t) t2 -> pm <> None ->
      Heap.MapsTo n (n1, t1) H2 -> well_typed_lit D U empty_theta H2 empty_scope n1 t1 ->
      (forall l h t', t2 = TPtr Tainted pm (TArray (Num l) (Num h) t') -> l <= n < h) ->
      (forall l h t', t2 = TPtr Tainted pm (TNTArray (Num l) (Num h) t') -> l <= n < h) ->
      @get_root D t2 tv ->
      step D U F s (H1,H2) (EDeref (ELit n (TPtr Tainted pm t))) s (H1,H2) (RExpr (ELit n1 tv))
...
(* Add two rules for when pm = None. *)
  ...
  | SDerefUnChecked : forall s H1 H2 m pm n n1 t1 t t2 tv,
      eval_type_bound s (TPtr m pm t) t2 ->
      Heap.MapsTo n (n1, t1) H2 ->
      m <> Checked ->
      @get_root D t2 tv ->
      step D U F s (H1,H2) (EDeref (ELit n (TPtr m pm t))) s (H1,H2) (RExpr (ELit n1 tv))
  ...
  | SAssignChecked : forall s H1 H2 n pm t na ta tv n1 t1 tv',
      Heap.MapsTo n (na,ta) H1 -> pm <> None ->
      eval_type_bound s (TPtr Checked pm t) tv ->
      (forall l h t', tv = TPtr Checked pm (TArray (Num l) (Num h) t') -> l <= n < h) -> 
      (forall l h t', tv = TPtr Checked pm (TNTArray (Num l) (Num h) t') -> l <= n < h) -> 
      @get_root D tv tv' ->
      step D U F
         s (H1,H2)  (EAssign (ELit n (TPtr Checked pm t)) (ELit n1 t1))
         s (Heap.add n (n1, ta) H1,H2) (RExpr (ELit n1 tv'))

  | SAssignTainted : forall s H1 H2 n pm t na ta tv n1 t1 tv',
      Heap.MapsTo n (na,ta) H2 -> pm <> None ->
          well_typed_lit D U empty_theta H2 empty_scope na ta ->
      eval_type_bound s (TPtr Tainted pm t) tv ->
      (forall l h t', tv = TPtr Tainted pm (TArray (Num l) (Num h) t') -> l <= n < h) -> 
      (forall l h t', tv = TPtr Tainted pm (TNTArray (Num l) (Num h) t') -> l <= n < h) -> 
      @get_root D tv tv' ->
      step D U F
...
(* Add two rules for when pm = None. *)
  ...
  | SAssignUnChecked : forall s H1 H2 m pm n t na ta tv n1 t1 tv',
  ...
  | SFieldAddrChecked : forall s H n pm t (fi : field) n0 t0 T fs i fi ti,
      n > 0 ->
      t = TPtr Checked pm (TStruct T) ->
      StructDef.MapsTo T fs D ->
      Fields.MapsTo fi ti fs ->
      List.nth_error (Fields.this fs) i = Some (fi, ti) ->
      n0 = n + Z.of_nat(i) ->
      t0 = TPtr Checked (add_pm pm i) ti ->
      word_type ti ->
      step D U F
  ...
  | SFieldAddrTainted : forall s H n pm t (fi : field) n0 t0 T fs i fi ti,
      n > 0 -> pm <> None ->
      t = TPtr Tainted pm (TStruct T) ->
      StructDef.MapsTo T fs D ->
      Fields.MapsTo fi ti fs ->
      List.nth_error (Fields.this fs) i = Some (fi, ti) ->
      n0 = n + Z.of_nat(i) ->
      t0 = TPtr Tainted pm ti ->
      word_type ti ->
      well_typed_lit D U empty_theta (snd H) empty_scope n0 t0 ->
      step D U F
         s H (EFieldAddr (ELit n t) fi)
         s H (RExpr (ELit n0 t0))

  | SFieldAddrNull : forall s H (fi : field) m n T pm,
      n <= 0 -> m <> Unchecked  ->
      step D U F
         s H (EFieldAddr (ELit n (TPtr m pm (TStruct T))) fi)
         s H RNull
  | SFieldAddr : forall s H n t (fi : field) n0 t0 T fs i fi ti pm,
      pm <>  None ->
      t = TPtr Unchecked pm (TStruct T) ->
      StructDef.MapsTo T fs D ->
      Fields.MapsTo fi ti fs ->
      List.nth_error (Fields.this fs) i = Some (fi, ti) ->
      n0 = n + Z.of_nat(i) ->
      t0 = TPtr Unchecked pm ti ->
      word_type ti ->
      step D U F
        s H (EFieldAddr (ELit n t) fi)
        s H (RExpr (ELit n0 t0))
  | SMallocChecked : forall s H1 H2 w w' H1' n1,
      ...
      step D U F
         ...
         s (H1',H2) (RExpr (ELit n1 (TPtr Checked (Some (HeapType,NumPtr n1)) w')))

  | SMallocUnChecked : forall s H1 H2 m w w' H2' n1 pm,
      eval_type_bound s w w' -> malloc_bound w' -> m <> Checked -> pm <> None ->
      allocate D H2 w' = Some (n1, H2') ->
      step D U F
         s (H1,H2) (EMalloc m w)
         s (H1,H2') (RExpr (ELit n1 (TPtr m pm w')))
  ...
  | SUnchecked : forall s H n,
      step D U F s H (EUnchecked (ELit n TNat)) s H (RExpr (ELit n TNat))
   | SIfDefTrueNotNTHit : forall s H x n t e1 e2 n1 t1, 
           Stack.MapsTo x (n,t) s ->
           step D U F s H (EDeref (ELit n t)) s H (RExpr (ELit n1 t1)) ->
           n1 <> 0 -> ~ (NTHit s x) -> step D U F s H (EIfDef x e1 e2) s H (RExpr e1)
   | SIfDefTrueNTHit : forall s H x n t e1 e2 n1 t1, 
           Stack.MapsTo x (n,t) s ->
           step D U F s H (EDeref (ELit n t)) s H (RExpr (ELit n1 t1)) ->
           n1 <> 0 -> (NTHit s x) -> step D U F s H (EIfDef x e1 e2) (add_nt_one s x) H (RExpr e1)
   | SIfDefFalse : forall s H x n t e1 e2 t1, 
           Stack.MapsTo x (n,t) s ->
           step D U F s H (EDeref (ELit n t)) s H (RExpr (ELit 0 t1)) ->
              step D U F s H (EIfDef x e1 e2) s H (RExpr e2)
   | SIfDefFail : forall s H x n t e1 e2 r,
           Stack.MapsTo x (n,t) s ->
              ~ is_rexpr r 
              -> step D U F s H (EDeref (ELit n t)) s H r
                 -> step D U F s H (EIfDef x e1 e2) s H r
   | SIfTrue : forall s H n t e1 e2, n <> 0 -> 
           step D U F s H (EIf (ELit n t) e1 e2) s H (RExpr e1)
   | SIfFalse : forall s H t e1 e2, 
              step D U F s H (EIf (ELit 0 t) e1 e2) s H (RExpr e2)
   | SIfEqTrue : forall s H n t t' e1 e2,
           step D U F s H (EIfPtrEq (ELit n t) (ELit n t') e1 e2) s H (RExpr e1)
  | SIfEqFalse : forall s H n n' t t' e1 e2, n <> n' ->
           step D U F s H (EIfPtrEq (ELit n t) (ELit n' t') e1 e2) s H (RExpr e2)
  | SIfLtTrue : forall s H n n' t t' e1 e2, n < n' ->
           step D U F s H (EIfPtrLt (ELit n t) (ELit n' t') e1 e2) s H (RExpr e1)
  | SIfLtFalse : forall s H n n' t t' e1 e2,  n' <= n ->
           step D U F s H (EIfPtrLt (ELit n t) (ELit n' t') e1 e2) s H (RExpr e2).


Hint Constructors step.

Inductive reduce (D : structdef) (U:union) 
...
Definition reduces (D : structdef) (U:union)  (F:Z -> option (list (var * type) * type * expression * mode)) 
...
Definition is_nt_ptr (t : type) : Prop :=
    match t with TPtr m pm (TNTArray l h t') => True 
              | _ => False
    end.
...
Inductive type_eq (S : stack) : type -> type -> Prop := 
     ...
     | type_eq_left: forall t1 t2, simple_type t1 -> eval_type_bound S t2 t1 -> type_eq S t2 t1
     | type_eq_right: forall t1 t2, simple_type t2 -> eval_type_bound S t1 t2 -> type_eq S t1 t2.

(* subtyping relation based on types. *)
Inductive subtype_stack (D: structdef) (U:union) (Q:theta) (S:stack) : type -> type -> Prop :=
     | subtype_same : forall t t', subtype D U Q t t' -> subtype_stack D U Q S t t'
     | subtype_left : forall t1 t2 t2', simple_type t1 -> eval_type_bound S t2 t2'
            -> subtype D U Q t1 t2' -> subtype_stack D U Q S t1 t2
     | subtype_right : forall t1 t1' t2, simple_type t2 -> eval_type_bound S t1 t1'
            -> subtype D U Q t1' t2 -> subtype_stack D U Q S t1 t2.

(* The join opeartions. *)
Inductive join_type (D : structdef) (U:union) (Q:theta) (S:stack) : type -> type -> type -> Prop :=
   join_type_front : forall a b, subtype_stack D U Q S a b -> join_type D U Q S a b b
  | join_type_end : forall a b, subtype_stack D U Q S b a -> join_type D U Q S a b a.
...
Inductive well_bound_vars_type {A:Type}: list (var * A) -> type -> Prop :=
  ...
  | well_bound_vars_ptr : forall l c pm t, well_bound_vars_type l t -> well_bound_vars_type l (TPtr c pm t)
...
Inductive well_typed_arg (D: structdef) (U:union) (Q:theta) (H : real_heap) (env:env): 
                mode -> expression -> type -> Prop :=
     | ArgLitChecked : forall n t t',
      simple_type t ->
      @well_typed_lit D U Q (fst H) empty_scope n t' ->
      subtype D U Q t' t ->
      well_typed_arg D U Q H env Checked (ELit n t') t
     | ArgLitUnChecked : forall n t t',
      simple_type t ->
      well_typed_arg D U Q H env Unchecked (ELit n t') t
     | ArgVar : forall m x t t',
      Env.MapsTo x t' env -> 
      well_type_bound_in env t ->
      subtype D U Q t' t ->
      well_typed_arg D U Q H env m (EVar x) t.

Inductive well_typed_args {D: structdef} {U:union} {Q:theta} {H : real_heap}: 
                   env -> mode -> list expression -> list (type) -> Prop :=
     | args_empty : forall env m, well_typed_args env m [] [].
(*
     | args_many : forall env m s e es v t vl, 
                 well_typed_arg D U Q H env m e (subst_type s t) ->
...
Definition NTHitVal (t:type) : Prop :=
   match t with | (TPtr m pm (TNTArray l (Num 0) t)) => True
                | _ => False
   end.

Definition add_nt_one_env (s : env) (x:var) : env :=
   match Env.find x s with | Some (TPtr m pm (TNTArray l (Num h) t)) 
                         => Env.add x (TPtr m pm (TNTArray l (Num (h+1)) t)) s
                              (* This following case will never happen since the type in a stack is always evaluated. *)
                             | _ => s
   end.
...
Definition match_mode_ptr (t:type) (m:mode) :Prop :=
    match t with TPtr m1 pm t => mode_leq m1 m
             | _ => True
    end.

Definition is_fun_type (t:type) :=
   match t with TFun x xl => True
             | _ => False
   end.
...
Inductive well_typed { D : structdef } {S:stack} {H:real_heap}
        : env -> union -> theta -> mode -> expression -> type -> Prop :=
  | TyLitChecked : forall env U Q n t,
      @well_typed_lit D U Q (fst H) empty_scope n t ->
      well_typed env U Q Checked (ELit n t) t
  | TyLitUnChecked : forall env U Q n t,
      well_typed env U Q Unchecked (ELit n t) t
  | TyVar : forall env U Q m x t,
      Env.MapsTo x t env ->
      well_typed env U Q m (EVar x) t

  | TyCall : forall env U Q pm m m' es x tvl t, 
  ...
  | TyStrlen : forall env U Q x pm m m' h l t, 
      Env.MapsTo x (TPtr m' pm (TNTArray h l t)) env ->
        mode_leq m' m ->
      well_typed env U Q m (EStrlen x) TNat

  | TyLetStrlen : forall env U Q pm m m' x y e l h t ta, 
      ~ Env.In x env ->
        mode_leq m' m ->
      Env.MapsTo y (TPtr m' pm (TNTArray l h ta)) env ->
      well_typed (Env.add x TNat (Env.add y (TPtr m' pm (TNTArray l (Var x 0) ta)) env)) U (Theta.add x GeZero Q) m e t ->
      ~ In x (get_tvars t) ->
      well_typed env U Q m (ELet x (EStrlen y) e) t

  | TyLetNat : forall env U Q m x e1 e2 t b,
      ~ Env.In x env ->
      well_typed env U Q m e1 TNat ->
      well_typed (Env.add x TNat env) U Q m e2 t ->
      In x (get_tvars t) -> get_good_dept e1 = Some b ->
      well_typed env U Q m (ELet x e1 e2) (subst_type [(x,b)] t)

  | TyLetPtrSame1 : forall env U Q ptr px m m' x e1 t1 e2 t,
      ~ Env.In x env ->
      well_typed env U Q m e1 (TPtr m' (Some (ptr,VarPtr px)) t) ->
      well_typed (Env.add x t1 env) (union_add U x px) Q m e2 t ->
      ~ In x (get_tvars t) ->
        mode_leq m' m ->
      well_typed env U Q m (ELet x e1 e2) t

  | TyLet : forall env U Q m x e1 t1 e2 t,
      ~ Env.In x env ->
      well_typed env U Q m e1 t1 ->
      well_typed (Env.add x t1 env) U Q m e2 t ->
      ~ In x (get_tvars t) ->
      well_typed env U Q m (ELet x e1 e2) t

  | TyRetTNat : forall env U Q m x na a e t,
      Env.In x env ->
      In x (get_tvars t) ->
      well_typed env U Q m e t ->
      well_typed env U Q m (ERet x (na,TNat) a e) (subst_type [(x,(Num na))] t)

  | TyRet : forall env U Q m x na ta a e t,
      Env.In x env ->
      well_typed env U Q m e t ->
      ~ In x (get_tvars t) ->
      well_typed env U Q m (ERet x (na,ta) a e) t

  | TyPlus : forall env U Q m e1 e2,
      well_typed env U Q m e1 (TNat) ->
      well_typed env U Q m e2 (TNat) ->
      well_typed env U Q m (EPlus e1 e2) TNat
  | TyPlusIndex : forall env U Q pm m t e1 e2,
      ~ is_fun_type t ->
      well_typed env U Q m e1 (TPtr m pm t) ->
      well_typed env U Q m e2 (TNat) ->
      well_typed env U Q m (EPlus e1 e2) (TPtr m pm t)
(*
...
*)
  | TyMallocChecked : forall env x m U Q w t e2,
      ~ Env.In x env ->
      mode_leq m Checked ->
      well_type_bound_in env w ->
      well_typed (Env.add x (TPtr Checked (Some (HeapType, VarPtr x)) w) env) (([x])::U) Q Checked e2 t ->
      well_typed env U Q Checked (ELet x (EMalloc m w) e2) t

  | TyUnchecked : forall env U Q m e,
      well_typed env U Q Unchecked e TNat ->
      well_typed env U Q m (EUnchecked e) TNat
  | TyCast1 : forall env U Q m t e t',
      well_type_bound_in env t ->
      match_mode_ptr t m ->
      well_typed env U Q m e t' ->
      well_typed env U Q m (ECast t e) t
  | TyCast2 : forall env U Q pm m m' t e t',
      well_type_bound_in env t ->
      well_typed env U Q Checked e t' -> 
      subtype_stack D U Q S t' (TPtr m' pm t) ->
      well_typed env U Q Checked (ECast (TPtr m None t) e) (TPtr m' pm t)

  | TyDynCast1 : forall env U Q pm m e x y u v t t',
      well_type_bound_in env (TPtr m pm (TArray x y t)) ->
      well_typed env U Q Checked e (TPtr m pm (TArray u v t')) ->
      type_eq S t t' ->
      mode_leq m Checked ->
      well_typed env U Q Checked (EDynCast (TPtr m None (TArray x y t)) e) (TPtr m pm (TArray x y t))
  | TyDynCast2 : forall env U Q pm m e x y t t',
      ~ is_array_ptr (TPtr Checked pm t') ->
      type_eq S t t' ->
      well_type_bound_in env (TPtr m pm (TArray x y t)) ->
      well_typed env U Q m e (TPtr Checked pm t') ->
      mode_leq m Checked ->
      well_typed env U Q Checked (EDynCast (TPtr m None (TArray x y t)) e) (TPtr m pm (TArray (Num 0) (Num 1) t))
  | TyDynCast3 : forall env U Q pm m e x y u v t t',
      well_type_bound_in env (TPtr m pm (TNTArray x y t)) ->
      type_eq S t t' ->
      well_typed env U Q Checked e (TPtr m pm (TNTArray u v t')) ->
      mode_leq m Checked ->
      well_typed env U Q Checked (EDynCast (TPtr m None (TNTArray x y t)) e) (TPtr m pm (TNTArray x y t))
  | TyDeref : forall env U Q pm m e m' t l h t' t'',
      well_typed env U Q m e t -> pm <> None ->
      subtype D U Q t (TPtr m' pm t'') -> 
      ((word_type t'' /\ t'' = t') 
       \/ (t'' = TArray l h t' /\ word_type t' /\ type_wf D  m' t')
       \/ (t'' = TNTArray l h t' /\ word_type t') /\ type_wf D  m' t') ->
      mode_leq m' m ->
      well_typed env U Q m (EDeref e) t'
(*
  ...
  | TyIndex2 : forall env Q m e1 m' l h e2 t,
      ...
      well_typed env Q m (EDeref (EPlus e1 e2)) t
*)
  | TyAssign1 : forall env U Q pm m e1 e2 m' t t1,
      subtype_stack D U Q S t1 t -> word_type t -> pm <> None ->
      well_typed env U Q m e1 (TPtr m' pm t) ->
      well_typed env U Q m e2 t1 ->
      mode_leq m' m ->
      well_typed env U Q m (EAssign e1 e2) t
  | TyAssign2 : forall env U Q pm m e1 e2 m' l h t t',
      word_type t -> type_wf D m' t -> subtype_stack D U Q S t' t ->
      well_typed env U Q m e1 (TPtr m' pm (TArray l h t)) -> pm <> None ->
      well_typed env U Q m e2 t' ->
      mode_leq m' m ->
      well_typed env U Q m (EAssign e1 e2) t
  | TyAssign3 : forall env  U Q pm m e1 e2 m' l h t t',
      word_type t -> type_wf D m' t -> 
     subtype_stack D U Q S t' t -> pm <> None ->
      well_typed env U Q m e1 (TPtr m' pm (TNTArray l h t)) ->
      well_typed env U Q m e2 t' ->
      mode_leq m' m ->
      well_typed env U Q m (EAssign e1 e2) t
(*
  ...
  | TyIndexAssign2 : forall env Q m e1 e2 e3 m' l h t t',
      ...
      well_typed env Q m (EAssign (EPlus e1 e2) e3) t
*)
  | TyIfDef : forall env U Q pm m m' x t t1 e1 e2 t2 t3 t4,
      Env.MapsTo x t env ->
      subtype D U Q t (TPtr m' pm t1) ->
      (exists l h t', (word_type t1 /\ t1 = t')
         \/ (t1 = TArray l h t' /\ word_type t' /\ type_wf D m' t')
       \/ (t1 = TNTArray l h t' /\ word_type t' /\ type_wf D m' t')) ->
      well_typed env U Q m e1 t2 -> well_typed env U Q m e2 t3 ->
      join_type D U Q S t2 t3 t4 -> 
      mode_leq m' m ->
      well_typed env U Q m (EIfDef x e1 e2) t4

  | TyIfDefNT : forall env U Q pm m m' x l t e1 e2 t2 t3 t4,
      Env.MapsTo x (TPtr m' pm (TNTArray l (Var x 0) t)) env ->
      well_typed (Env.add x (TPtr m' pm (TNTArray l (Var x 1) t)) env) U Q m e1 t2 
        -> well_typed env U Q m e2 t3 ->
      join_type D U Q S t2 t3 t4 -> 
      (m' = Unchecked -> m = Unchecked) ->
      well_typed env U Q m (EIfDef x e1 e2) t4


  | TyIf : forall env U Q m e1 e2 e3 t2 t3 t4,
      well_typed env U Q m e1 TNat ->
      well_typed env U Q m e2 t2 ->
      well_typed env U Q m e3 t3 ->
      join_type D U Q S t2 t3 t4 -> 
      well_typed env U Q m (EIf e1 e2 e3) t4. 
...
Definition env_denv_prop (env: env) (S:stack) (denv:dyn_env) :=
    forall x t t', Env.MapsTo x t env -> eval_type_bound S t t' -> Stack.MapsTo x t' denv.
*)
...
Lemma gen_eval_type_bound_same :
   forall env s t, well_type_bound_in env t -> sub_domain env s
           -> (exists t', eval_type_bound s t t').
Proof.
  intros. induction t.
  exists TNat. apply eval_type_bound_nat.
  inv H. apply IHt in H3. destruct H3.
  exists (TPtr m x). apply eval_type_bound_ptr. assumption.
  exists (TStruct s0). apply eval_type_bound_struct.
  ...
  apply eval_type_bound_ntarray.
  1 - 5: assumption.
Qed.


Lemma cast_word_type : forall s t t', eval_type_bound s t t' -> word_type t -> word_type t'.
Proof.
 intros. inv H0. inv H. constructor.
 inv H. constructor.
Qed.

Lemma cast_type_wf : forall D s t t', eval_type_bound s t t' -> type_wf D t -> type_wf D t'.
Proof.
...
Lemma eval_type_bound_same : forall s t t' t'',
...
Lemma simple_type_means_cast_same : forall t s, simple_type t -> eval_type_bound s t t.
Proof.
...
Lemma cast_type_eq : forall s t t1 t', 
        eval_type_bound s t t' -> type_eq s t t1 -> eval_type_bound s t1 t'.
Proof.
 intros. inv H0. easy.
 assert (t' = t1).
 eapply eval_type_bound_same. apply H. easy. subst.
 apply simple_type_means_cast_same. easy.
 assert (t1 = t').
 specialize (simple_type_means_cast_same t1 s H1) as eq1.
 eapply eval_type_bound_same. apply H2. easy. subst.
 apply simple_type_means_cast_same. easy.
Qed.
...
Lemma empty_env_means_eval_type_bound_same :
   forall s t, well_type_bound_in empty_env t -> eval_type_bound s t t.
Proof.
 intros. 
...
(*
Lemma lit_empty_means_eval_type_bound_same :
  forall D F Q S H m n t t1, @well_typed D F S H empty_env Q m (ELit n t) t1 ->  eval_type_bound S t t.
Proof.
 intros. remember empty_env as env.
 inv H0.
...
Proof.
 ...
 assert (exists t', eval_type_bound S t t').
...
Lemma well_type_bound_vars_cast : forall t l s, sub_list_arg_s l s -> well_bound_vars_type l t ->
               no_etype t -> (exists t', eval_type_bound s t t' /\ simple_type t').
Proof.
...
Lemma cast_type_no_etype : forall s t t', eval_type_bound s t t' -> no_etype t' -> no_etype t.
Proof.
  intros. induction H;simpl. easy.
  inv H0. constructor. apply IHeval_type_bound. easy.
  inv H0. constructor. apply IHeval_type_bound. easy.
...
Lemma cast_type_no_etype_r : forall s t t', eval_type_bound s t t' -> no_etype t -> no_etype t'.
Proof.
  intros. induction H;simpl. easy.
  inv H0. constructor. apply IHeval_type_bound. easy.
  inv H0. constructor. apply IHeval_type_bound. easy.
...
Lemma cast_means_simple_type : forall s t t', eval_type_bound s t t' -> simple_type t'.
Proof.
  intros. induction H. 
  ...
  | WFS_Stack : forall env s,
   ...
   /\ (forall x v t,
             Stack.MapsTo x (v, t) s -> 
                   @well_typed_lit D H empty_scope v t    
          -> exists t' t'',
                @Env.MapsTo type x t' env /\ eval_type_bound s t' t''
                   /\ subtype D Q t t'')
 ->
...
Lemma cast_ptr_left : forall s t1 m t2, eval_type_bound s t1 (TPtr m t2) -> exists ta, t1 = (TPtr m ta).
Proof.
  intros. inv H. exists t. easy.
Qed.
...
Lemma stack_grow_cast_type_same : forall env S S' t t', well_type_bound_in env t ->
        sub_domain env S -> stack_consistent_grow S S' env ->
              eval_type_bound S t t' -> eval_type_bound S' t t'.
Proof.
  intros.
  induction H2. constructor.
  constructor.
  apply IHeval_type_bound. inv H. easy.
  inv H.
  constructor.
  apply (stack_grow_cast_same env0 S); try easy.
  apply (stack_grow_cast_same env0 S); try easy.
  apply IHeval_type_bound. easy.
  inv H.
  constructor.
  apply (stack_grow_cast_same env0 S); try easy.
  apply (stack_grow_cast_same env0 S); try easy.
  apply IHeval_type_bound. easy.
  apply eval_type_bound_struct.
Qed.
...
Lemma cast_subtype_same : forall t1 D Q S t1' t2 t2', subtype D Q t1 t2 -> 
    eval_type_bound S t1 t1' -> eval_type_bound S t2 t2' ->
...
Proof.
  ...
  inv H0.
  inv H. inv H1.
  specialize (eval_type_bound_same S t1 t' t'0 H6 H4) as eq3. subst.
  constructor. inv H5. inv H8.
  inv H1. inv H8. unfold cast_bound in *. inv H7. inv H10.
  specialize (eval_type_bound_same S t1 t' t'1 H6 H11) as eq3. subst.
  apply SubTyBot.
  ...
  specialize (eval_type_bound_same S t1 t' t'1 H6 H12) as eq3. subst.
  apply SubTyBot.
...
Lemma cast_as_same : forall t S S' env AS t',
    ...
    (forall a, In a AS -> ~ Env.In (fst a) env) ->
     ...
     (forall x b na ta, In (x,b) AS -> Stack.MapsTo x (na,ta) S' -> bound_eval S b = Some na) ->
    (forall n n' a b, n <> n' -> nth_error AS n = Some a -> nth_error AS n' = Some b -> fst a <> fst b) ->
     eval_type_bound S (subst_type AS t) t' ->
     eval_type_bound S' t t'.
Proof.
   induction t; intros; simpl.
...
Lemma stack_wf_out : forall tvl es D Q H env AS S S',
     ...
     (forall x n t ta, Env.MapsTo x t env -> Stack.MapsTo x (n,ta) S ->
           (exists t', eval_type_bound S t t' /\ subtype D Q ta t')) ->
     (forall x n ta, Theta.MapsTo x GeZero Q -> Stack.MapsTo x (n,ta) S -> 0 <= n) ->
     (forall n n' a b, n <> n' -> nth_error tvl n = Some a -> nth_error tvl n' = Some b -> fst a <> fst b) ->
...
Proof.
  ...
  split. easy. 
  split.
  apply (eval_type_bound_same s (subst_type AS t) (subst_type AS t) t' eq1) in H20.
  ...
  specialize (gen_eval_type_bound_same env0 s t'0 H11 H0) as eq1.
  destruct eq1.
  ...
  specialize (eval_type_bound_same s t'0 x1 x2 H16 H17) as eq3. subst.
...
Proof.
  ...
  apply eval_type_bound_same with (t' := (subst_type AS t)) in H18; try easy.
  rewrite <- H18.
  apply subtype_well_type with (t := t'0); try easy.
...
Lemma eval_type_bound_not_nat :
   forall env S t t' x n ta m tb, sub_domain env S -> well_type_bound_in env t ->
     Env.MapsTo x (TPtr m tb) env ->
     eval_type_bound S t t' -> 
     eval_type_bound (Stack.add x (n,ta) S) t t'.
Proof.
  intros. induction H2. constructor.
  constructor. apply IHeval_type_bound. inv H0. easy.
  ...
  inv H0. easy.
  apply IHeval_type_bound. inv H0. easy.
  constructor. apply cast_bound_not_nat with (env := env0) (m := m) (tb := tb); try easy.
  inv H0. easy.
  apply cast_bound_not_nat with (env := env0) (m := m) (tb := tb); try easy.
  inv H0. easy.
