...
Lemma well_typed_null_array D H s n w :
  well_typed_lit D H s n (TPtr Checked (TArray 0 w)).
Proof.
  eapply TyLitC; simpl; eauto; simpl.
  intros k [? Contra].
  inv Contra.
Qed.
...
(*
Lemma repl_same {A} : forall (n k : nat) (x : A),
  nth_error (x :: replicate n x) k = Some x ->
  nth_error (replicate n x) k = Some x.
Proof.
  intro n; induction n; intros k x H; simpl in *; auto.
  destruct n; simpl in *; auto.
  destruct (replicate n x) eqn:HR; simpl in *; auto.
  - 
  *)  
...
Proof.
  ...
  - split.
    ...
    * unfold allocate in H1.
    ...
    + exists 0; exists w.
      destruct n eqn:Hn; simpl in *; try omega.
      split; auto.
      + apply Hyp in HOrd.
        { destruct HOrd as [[n' t'] HM'].
          destruct (length_nth (replicate n w) (S k)) as [x Hnth].
          - rewrite replicate_length in *. omega.
          - exists n'. exists x.
(*            assert (Hk' : O <= k < length (replicate n w)) by omega. *)
    ...
    + clear H1.
      ...
      * { (* Plus step *)
          ...
          - inv HTy1.
            ...
            destruct n.
            { (* New contra case *)
              eapply TyLitC; intros; simpl in *; eauto.
...
*)
            ...
            + remember (S n - n2) as m eqn:HN.

              eapply TyLitC with (ts := replicate m t) ; eauto.
(*              * destruct m; intros; try omega; simpl in *; eauto.
                eexists; exists t.
                split; eauto.
                specialize (H7 k H1).
                intros. destruct m; [omega | ]; simpl in *; eauto.
                eexists. exists t.
                split; eauto.*)
