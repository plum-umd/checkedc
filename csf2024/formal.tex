...
\evalue{n}{\tau} \mid x \mid \ebinop{e}{e}\mid \ecast{\tau}{e} \mid \estar{e}\mid\eassign{e}{e} 
  \\[0.2em]
&&\mid& \textcolor{red}{\emalloc{\xi}{\omega}} \mid \textcolor{red}{\toa{m}{\overline{\evalue{x}{\tau}}}{\tau}{e}} \\[0.2em]
...
\mid \textcolor{red}{\toa{m}{\overline{\evalue{x}{\tau}}}{\tau}{E}}\\[0.1em]
...
shortly. Compared with \checkedc, we add three marked red contexts in \Cref{fig:c-context}.
$\ecall{E}{\overline{e}}$ and $\ecall{(\evalue{n}{\tau})}{\overline{E}}$ are the contexts for function calls,
since every call requires a function pointer having the address $n$ here,
while the \checkedc formalism's function call takes a global function variable name \cite{li22checkedc}.
$\toa{m}{E}$ is the context form of mode switching expression $\toa{m}{e}$.

The $\mode$ function
determines the mode when evaluating $e'$ based on the context $E$:
if the $\Box$ occurs within $\toa{m}{E'}$ inside $E$ and $E'$ does not contain another $\totext$ expression, then the mode is
$\umode$; otherwise, it is $\cmode$. Evaluation contexts $E$ define a
standard left-to-right evaluation order.
Two expressions $\ret{x}{\mu}{e}$ and $\toa{m}{e}$, which does not appear in source program syntax in \Cref{fig:checkc-syn}, are introduced in evaluating programs. The former is used for remembers $x$ was previously bound to
$\varphi(x)$ in a $\elettext$ binding, which is described in \citet{li22checkedc} and \Cref{app:main}.
The latter ($\toa{m}{e}$) is a new $\lang$ syntax for switching the context mode in $e$ as $m$, which is generated when a function is called; explained shortly below.
...
{\small
  \begin{mathpar}
    \inferrule[S-DefArray]{n\in[\eta_{\xi},|\heap(\xi)|)\\\heap(\xi,n)=\evalue{n_a}{\tau_a} \\ 0 \in [n_l,n_h)}
    {(\varphi,\heap,\estar{\evalue{n}{\tntarrayptr{n_l}{n_h}{\tau}{\xi}}}) \longrightarrow (\varphi,\heap,\evalue{n_a}{\tau})}

        \inferrule[S-Def]{\heap(n)=\evalue{n_a}{\tau_a} \\ n\in[\eta_{\xi},|\heap(\xi)|)}
    {(\varphi,\heap,\estar{\evalue{n}{\tptr{\tau}{\xi}}}) \longrightarrow (\varphi,\heap,\evalue{n_a}{\tau})}

    \inferrule[S-DefArrayBound]{0 \not\in [n_l,n_h)}
     { (\varphi,\heap,\estar{\evalue{n}{\tallarrayptr{n_l}{n_h}{\tau}{\xi}}}) \longrightarrow (\varphi,\heap,\ebounds)}

    \inferrule[S-DefNull]{n\not\in[\eta_{\xi},|\heap(\xi)|)}{(\varphi,\heap,\estar{\evalue{n}{\tptr{\omega}{\xi}}}) \longrightarrow (\varphi,\heap,\enull)}
