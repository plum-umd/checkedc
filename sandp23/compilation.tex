...
\Cref{fig:flagtable} lists the different compiled syntax for a deference operation ($\estar{x}$) for the compiler implementation (\text{Impl}) and formalism (\text{Foml}). The columns represent different pointer modes and the rows represent context modes.
...
In the compilation, we use a function \code{mode} to get the pointer mode of a function learning from the pointer's type.
...
we place bounds and null-pointer checks, such as the line 6 and 7 in \Cref{fig:compilationexample1}.
In addition, in the formalism, before every use of a tainted pointer (\Cref{fig:compilationexample1} line 9), 
...
Fpr example, in compiling the \code{stringsort} function in \Cref{fig:checkedc-example-1},
we place a check \code{verify_fun(cmp, not_null && type_match)},
...
\Cref{fig:compilationexample1} also shows an example.
