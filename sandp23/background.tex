...
\liyi{Do we need to spend more than half page introducing checked-c? OR just introduce it briefly like what we had in the formalism paper? }
...
until a~\code{NULL} terminator is reached (\ie the~\code{NULL} is not within the bounds).

\liyi{can we reuse the old text for the spatial safety? }
...
For instance, in the code \inlinecode{if (n>0) a[n-1] =} $...$ the write
...
\code{checked} keyword.\footnote{You can also designate~\emph{unchecked} regions  within checked ones.}
 Within checked regions, both legacy pointers and
certain unsafe idioms (\eg \emph{variadic} function calls) are disallowed.

\liyi{If we are not going to talk about converting C to \checkedc, can we just move this to future work? }
...
\liyi{Does the program too small? Will reviewer complain about the program size? }
\subsection{No Safety Against Unchecked Code}
\label{subsec:nosafetyagsintuncheckedcode}
The spatial safety guarantee~\checkedc provides applies to completely converted programs,~\ie programs that uses~\emph{only} checked types and no regular pointer types.
A partially annotated program can still enjoy spatial safety but only on those code regions using only Checked pointers (\ie checked or safe regions). For instance, in the below example, there can be no spatial safety violations in the function~\code{func} as it uses only checked pointers.
...
\end{minted}
Here, the checked function~\inlinecode{func} expected a pointer to a buffer of five elements, but unchecked code violated it and invoked the function (Line 9) with a buffer of 2 elements.
...
Furthermore, since checked and unchecked regions execute in the same address space, spatial memory corruptions (\eg buffer overflow) in unchecked regions can take down the complete program despite having checked regions.
