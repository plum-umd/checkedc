...
\mz{
  Might as well state separately that dynamic checkes are instructmented upon
...
\mzr{                           %
  ...
  % 
  Continuing with the example in \Cref{fig:checkedc-example-1}, we can find out
  that \code{cmp} argument in \code{stringsort} is not exactly the same with the
  signature of \code{cmp_sum}: \code{cmp} has type
  \code{ptr<(int)(nt_array_ptr<char>, nt_array_ptr<char>)>} which takes stwo
  NT-array pointers with arbitrary size and outputs an integer, but
  \code{cmp_sum} \uline{ (Wait, what's the exact difference?) We may need to
    make it clear here}. 
  %
  To make function pointers usable, we also defines a subtyping relation in
  \systemname{} that permits function pointer subsumption.
  %
  In this example, the of type \code{cmp_sum} is automatically casted to the
  type \code{cmp}. 
  % 
...
respectively.  In the tainted shells, \mzr{the types of} \mzs{arguments}
\mzr{parameters} are the tainted \mzr{counterparts} \mzs{\versions of the
  corresponding arguments} in the \mzr{original} checked \mzs{functions}
\mzr{ones}.  
% 
Inside the shell body, \mzr{the compiler} \mzs{we} \mzu{creates checked pointer
  ...
  part is unclear. What do you call copy? Copying pointers doesn't make sense:
  pointers are just memory address. It's copying data that really helps. } 
% 
...
\Cref{fig:checkedc-example-3} line 42 is an example of serving the function call
living in an unchecked region with a tainted shell pointer argument
\code{tainted_compare_1}.  Even if $g$ derandomizes its address (line 14), the
shell address is in the sandbox and has no harm, and calling the shell never
exposes any checked pointer information outside of the shell.
% 
...
called, the system is turned to a checked region.  For example,
\Cref{fig:checkedc-example-2} line 25 surrounds the tainted shell body with a
$\echeckedtext$ block indicating that we context-switch from the unchecked to
checked code region; thus, the checked function call at
...
checked block containing the shell body.  We also make sure that no arguments in
these tainted shell contains any checked pointers, as well as no output is of a
checked type.
%
