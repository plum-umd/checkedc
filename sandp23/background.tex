...
\begin{itemize}
\item \code{ptr<}$T$\code{>} ($\ptr$) types a pointer that is either null or
  points to a single object of type $T$.
\item \code{array_ptr<}$T$\code{>} ($\arr$) types a pointer that is either null
  or points to an array of $T$ objects. The array width is defined
  by a \emph{bounds} expression, discussed below.
\item \code{nt_array_ptr<}$T$\code{>} ($\ntarr$) is like
...
bounds of \code{a} are \code{count(u)}, the
...
\liyi{I do not think you need three sub-sections for these topics. They can be in the same place. }
