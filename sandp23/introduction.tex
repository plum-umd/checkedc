...
have investigated ways to
\mzu{compile C programs 
  to automatic spatial safety enforcement.}
\mz{You cannot compile a program to enforcement.}
% 
...
% 
\mzs{ with new types and annotations to ensure a program’s spatial safety.
  Specifically, \checkedc extends C with checked pointer types, which are
  restricted by the compiler to spatially safe uses.}
% 
\mzr{to ensure a program’s spatial safety by introducing additional annotations and
  enforcing new types, checked pointer types in particular.}
% 
\mz{Redundancy?}
% 
The checked pointers are represented as system-level memory words\mzs{,~\ie} without ``fattening'' metadata, ensuring backward compatibility.
\mz{\ie{} sounds indirect and redundant to me.}
% 
\mzu{This enables incremental conversion, where regions of code can be converted to~\checkedc and ensure spatial safety within those regions.}
\mz{This sentence is incoherent. I don't understand the connection between
  incremental conversion and regions of code. 
  % 
  I guess your point is that since you don't introduce extra metadata, 
  checked and unchecked region can coexist and interact seamlessly.
  % 
}
% 
However, as we explain in~\sect{subsec:nosafetyagsintuncheckedcode}, the
unconverted or unchecked code can violate guarantees provided in \mzu{Checked}
\mz{either tt it or use lowercase?}
regions.
%
...
Existing mechanisms are not suitable as they are based on process isolation and have high overhead, and are data-centric~\emph{data-centric} \lc{duplicate?}(\sect{subsec:background:programpart}).
...
% 
\mz{It sounds hard to follow. The subjects in these sentences are  ``our
  system''. ``checked types'' and ``our type-system''.
  %
  For readers, the connection among checked, tainted and unchecked seems weak.
  % 
} 
\lc{it's  a little bit hard to follow. it talks about tainted -> checked -> unsafe interactions but doesn't explain how these are related}
% 
...
% 
The developer starts by marking desired (\ie unchecked) functions and pointers used in these functions as tainted. \lc{what's "these functions" in "used in these functions as tainted"? (and "Second" seems wired here)}
Second,~\systemname partitions the given program into two partitions (\umode and~\cmode regions)  of different \mzu{privileges}:
% 
\mz{need a bit more description (4-5 words) before introducing the word
  ``privilege''}
% 
...
\mz{                            %
  ...
  % 
  How does regions look like? Can one region nested in another?
  %
  If not, ``This partition contains the remaining (untainted) code and data and
  has complete access to~\cmode region.'' creates a sense of recursive
  definition of \cregion.
}\lc{<- same question, maybe giving an abstract example?}
% 
...
% 
\mzu{As functions in the~\ucregion can only access tainted types (or unchecked
types),~\cregion functions should use tainted types to pass pointer arguments
to~\ucregion functions.
% 
We avoid marshaling by allocating all tainted pointers (\ie tainted buffers)
in~\ucregion and thus can be accessed in both partitions.}
%
\mz{I don't quite understand the logical consequence here. Why does allocating
  tainted points avoids marchaling. 
  % 
  I see that ``allocating all tainted pointers'' makes ptrs accessable in both
  regions, but avoiding marchaling doesn't seem to cause that.
  %
  The question I should probably ask is what marshaling means? 
  % 
  Does the procedure that copies a checked buffer to a tainted one counts as
  marshaling?
  % 
}
% 
...
We handle this by using indirection. Specifically, instead of directly accessing the~\cregion callbacks, the ~\ucregion accesses them using a protected trampoline function, which will direct the execution to the appropriate callback function.
In addition, the trampoline function itself will be referenced using an opaque index rather than its virtual address, and \mzu{we will utilize existing sandboxing techniques to implement this}.\mz{tense?}
% 
...
We evaluated~\systemname by partioning~\numprog{} large real-world programs to demonstrate its effectiveness.
