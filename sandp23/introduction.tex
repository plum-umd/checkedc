...
\liyi{why we need to mention C to \checkedc conversion? Is this paper about introducing CheckedCBox or about the conversion from C to CheckedCBox. I am actually against putting CheckedCBox conversion stuff in the paper, because it is kind of confusion. We have the compilation from CheckedCBox to LLVM and have the conversion from C to CheckedCBox. }
...
However, as we explain in~\sect{subsec:nosafetyagsintuncheckedcode}, the unconverted or unchecked code can violate guarantees provided in Checked regions.
%
...
\liyi{Can we strengthen the sentence a little by saying something fancy? }
...
\liyi{can we just name them unchecked and checked regions, and then referring to them as \ucregion and~\cregion? We need to make it clear that we are re-define c and uc regions. }
\begin{itemize}
\item~\ucregion or~\emph{tainted region} (low privilege tainted region): This partition contains tainted types (\ie functions and pointers) and can only access tainted pointers.
...
In addition, the trampoline function itself will be referenced using an opaque index rather than its virtual address, and we will utilize existing sandboxing techniques to implement this.
