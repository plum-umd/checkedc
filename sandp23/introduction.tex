...
violation.
Such a state can always be attributed to, i.e., 
\emph{blamed on}, the execution of code that is not in a checked core region. 
However, what are the guarantees that unchecked (unsafe) code regions can provide?
...
\mz{This paragraph sounds incoherent to me.}
% 
Our first contribution is a clear program partition between checked and
unchecked code regions \mzs{and ensures} \mzr{so} that a \systemname program execution
\textit{cannot crash} due to spatial safety violations.
\mzr{
  % 
  In \systemname, heap is partitioned into two parts: a \textit{checked heap}
  containing all checked pointers and an \textit{unchecked heap} that is
  sandboxed.
  % 
  Then, we restricted pointer usages so that checked pointers can only refer to
  checked heap region and unchecked ones can only be used in unchecked heap
  region.
  %
  To communicate safely between those two isolated regions, we developed tainted
  pointers that is always verified upon accessing data and program in unchecked
  region.
  % 
}
\mzs{ 
  % 
  We restrict the checked and unchecked pointers usage only in checked and
  unchecked code regions, respectively; and develop tainted pointers for the
  communication between the two regions.  Heap is partitioned into two parts: a
  \textit{checked heap} containing all checked pointers and an \textit{unchecked
    heap} that is sandboxed.  Tainted pointers exist in the unchecked heap
  region and must be verified each time being used.
  % 
}
...
compiler. We create a program random generator 
based on the
typing rules of \lang and ensure that \lang and Clang Checked C are consistent after conversion, both statically and
dynamically.  
To the best of our knowledge, 
\lang is the first C(-like) language and compiler formalism with the program partitioning mechanism.
