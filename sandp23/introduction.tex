...
One of the main reasons for this is combining partitioning with~\acf{SFI}.
...
But existing partitioning techniques are specialized for a specific~\ac{SFI}~\cite{wahbe1993efficient} technique.
...
In summary, existing techniques combine policy (\ie{}~\emph{what} to partition) with mechanism (\ie{}~\emph{how} to partition).
...
For instance,~\isoheap{} provides safety against spatial and temporal violations of tainted pointers with almost no performance overhead.
