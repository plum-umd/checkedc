...
We will present our implementation using WASM~\cite{bosamiya2020webassembly} as our target sandbox.
...
However, we use a small OCAML program (\numocamllines{}) to remove annotations and make the code compilable with the sandbox compiler (CITE CORRESPONDING IMPL SECTION).
...
We rewrite all calls to tainted functions with corresponding sandbox invocations.
For instance, we modify the call to~\code{process_req1} in~\lst{lst:final} as below:
\begin{minted}[mathescape, escapeinside=||, fontsize=\footnotesize]{diff}
case REQ1: 
- rc = process_req1(buff); 
+ rc = .....
\end{minted}
\aravind{Arun, please fix the above example with the exact details.}
...
\begin{minted}[mathescape, escapeinside=||, fontsize=\footnotesize]{c}
_Callback _TPtr<char> StringAuth(_T_Array_Ptr<const char> usertoken : count(len),
                                 size_t len) {
...
...
Note that the function accepts a tainted array pointer as its parameter, enabling~\ucregion (tainted) region to invoke it.
...
The trampoline function performs the necessary conversion of pointer arguments and eventually invokes the callback.
