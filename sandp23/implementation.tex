...
This qualifier permits copying the corresponding function into both ~\cregion and~\ucregion, which permits the handling of certain simple utility functions that might be called from both regions. For example, ~\code{append_string} in our evaluation of parsons\_wasm has callers from both the regions. 
...
\end{minted}
Qualifying ~\code{append_string} with~\code{_Mirror} duplicates the function in both regions,
so that calling \code{append_string} permits the avoidance of over-tainting \code{appendStr},
as \code{appendStr} can be unchecked and checked pointers in $\umode$ and $\cmode$ regions, respectively.
"\_Mirror" enforces control-flow and data-flow compile-time semantic rules as shown above to assert region locality and exclusivity amongst between both the regions. \liyi{fix the sentence.}

\noindent\emph{\_TLIB}:
This qualifier relaxes type-checking rules on library functions without memory modification allowing developers to use the function freely in ~\cregion.
\begin{minted}[mathescape, escapeinside=||, fontsize=\footnotesize]{c}
...
Passing tainted pointer ~\code{t_str} to unqualified ~\code{strncat} above is disallowed by our type-checker as it expects both the pointers to be non-tainted since we are in ~\cregion. If the developer knows that~\code{strncat} accesses only~\code{n} bytes from the source buffer, she would like to directly use ~\code{strncat} with tainted source pointer in ~\cregion, instead of implementing the logic for string concatenation in ~\ucregion or marshalling data between the two regions.  
