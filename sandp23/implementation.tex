...
Passing tainted pointer \code{t_str} to unqualified ~\code{strncat} above is disallowed without having additional sandboxing for \code{t_str}. If a user ascertains that \code{t_str} has the right buffer size for \code{strncat}, she might label \code{strncat} with \code{\_TLIB}, so that \code{t_str} is treated as an unchecked pointer; such annotation relaxes type-checking for all the arguments to its calls. It is worth noting that \systemname does not enforce any semantics to ensure ~\code{\_TLIB} functions implemented in ~\cregion are non memory-modifying; therefore, using \code{\_TLIB} requires users' awareness of memory address leaks.
