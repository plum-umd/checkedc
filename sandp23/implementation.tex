...
The~\sourcerewriter is primarily implemented in C++ as a~\clang frontend tool (\numcheckmatelines{3028}).
However, we use a small OCAML program (\numocamllines{680}) to remove annotations and make the code compilable with the sandbox compiler (\sect{subsubsec:genucregion}).
...
This helps handle certain simple utility functions that might be called from both regions. For example, ~\code{append_string} in our evaluation of parsons\_wasm has callers from both the regions. 
...
\end{minted}
Placing ~\code{append_string} in ~\ucregion would require marking ~\code{appendStr} as tainted, thereby, requiring callers from ~\cregion to pass tainted parameter to ~\code{appendStr} at call-site. This would require either costly marshalling or making the arguments and other dependent pointers tainted, leading to over-tainting.
Similarly, placing ~\code{append_string} only in ~\cregion will make it hard for the callers in ~\ucregion as they should now use the callback mechanism to invoke~\code{append_string}.
Qualifying ~\code{append_string} with~\code{_Mirror} makes ~\sourcerewriter duplicate the function to ~\ucregion so that callers in each region will invoke their corresponding local copy, thereby, avoiding unnecessary tainting. 
"\_Mirror" enforces control-flow and data-flow compile-time semantic rules as shown above to assert region locality and exclusivity amongst between both the regions.

\noindent\emph{\_TLIB}:
This qualifier relaxes type-checking rules on non-memory modifying library functions allowing developers to use the function freely in ~\cregion.
\begin{minted}[mathescape, escapeinside=||, fontsize=\footnotesize]{c}
...
In the above code, passing tainted pointer ~\code{t_str} to unqualified ~\code{strncat} is disallowed by our type-checker as it expects both the pointers to be non-tainted as we are in ~\cregion. If the developer knows that~\code{strncat} accesses only~\code{n} bytes from the source buffer, she would like to directly use ~\code{strncat} with tainted source pointer in ~\cregion, instead of implementing the logic for string concatenation in ~\ucregion or marshalling data between the two regions.  
...
Consequently, annotating ~\code{strncat} as shown above relaxes type-checking for all the arguments to its calls. However, \systemname does not enforce any semantics to ensure ~\code{\_TLIB} functions implemented in ~\cregion are non memory-modifying, thereby, requiring developers to be careful to while using ~\code{\_TLIB} to prevent memory address leaks.
