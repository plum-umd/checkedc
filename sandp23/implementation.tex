...
\noindent\emph{Handling Tainted Functions}:
In~\cregion, we also need to modify tainted functions as they need to execute inside the sandbox (separate address space).
Consequently, We rewrite the tainted function's body with its corresponding sandbox invocation, thereby, routing all ~\cregion calls to tainted functions to their respective sandboxed implementations in ~\ucregion.
For instance, we modify the function body of ~\code{process_req1} in~\lst{lst:final} as below:
\begin{minted}[mathescape, escapeinside=||, fontsize=\footnotesize]{diff}
...
The~\code{read_msg} function signature in \lst{lst:final} will be modified as shown below in the ~\ucregion:
\begin{minted}[mathescape, escapeinside=||, fontsize=\tiny]{diff}
