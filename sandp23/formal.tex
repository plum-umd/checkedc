...
theories, including the type soundness, non-exposure, and non-crashing theorems.
...
It prevents unsafe types from being constructed.
  ...
  Syntactically, this type is however \emph{illegal} in~\lang{} and
  is rejected by the well-formedness. 
...
\end{array}
\]
The function type also has well-formed requirements (\Cref{app:le}), which disallows nesting checked pointers inside tainted pointers.
...
In \Cref{lst:humantaint}, the function call \code{handle_request} has $\umode$ context mode (is in $\umode$ region) since it is inside an unchecked function \code{server_loop}.
