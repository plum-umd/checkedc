...
\Cref{fig:checkc-syn} shows the syntax of~\systemname.
...
It prevents unsafe types from being constructed.
  Consider the type ~\code{_t_Array_ptr<_Ptr<int>>}, which describes a tainted array of checked pointers.
...
\systemname expressions include common expressions such as addition ($\ebinop{e_1}{e_2}$), 
...
In \systemname, Context modes $m$ appearing in a type rule determine the code region 
...
\systemname also provides static casting operations. As described in rule \textsc{T-CastPtr} in \Cref{fig:type-system-1},
...
Notice that \texttt{let} statements are immutable in \systemname, so the following code is not possible, because variables \code{x} and \code{y} must have the same type in \systemname.
