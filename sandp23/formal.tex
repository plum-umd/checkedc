...
$\textcolor{blue}{\text{Dereference Operations:}}\\$
  \begin{mathpar}
    \inferrule[T-Def]
        ...
        \inferrule[S-DefT]{\heap(\tmode,n)=\evalue{n_a}{\tau_a} \\ n \not\in \Theta \\ n < |\heap(\tmode)|}
    {(\varphi,\Theta,\heap,\estar{\evalue{n}{\tptr{\tau}{\tmode}}}) \longrightarrow (\varphi,\Theta,\heap,\evalue{n_a}{\tau})}

        \inferrule[S-Def]{\xi \neq \tmode \\ \heap(\xi,n)=\evalue{n_a}{\tau_a} }
    ...
    \inferrule[S-DefBound]{n \in \Theta \vee n \ge |\heap(\tmode)|}{(\varphi,\Theta,\heap,\estar{\evalue{n}{\tptr{\tau}{\tmode}}}) \longrightarrow (\varphi,\Theta,\heap,\ebounds)}
  \end{mathpar}
...
and $\heap$ is a \emph{heap} that is partitioned into three parts ($\cmode$, $\tmode$, and $\umode$ heap regions), each of which
maps addresses (integer literals) to values $\evalue{n}{\tau}$.
We wrote $\heap(m)$ to retrieve the $m$ heap and $\heap(m,n)$ to retrieve the $n$-location heap value in the $m$ heap.
...
The type and semantic rules for pointer dereference (\rulelab{T-Def}, \rulelab{S-DefT},
\rulelab{S-Def}, \rulelab{S-DefBound}, \rulelab{S-DefNull} in \Cref{fig:type-system-1})
...
Operation $\heap(\xi,n)$ not only accesses the $n$-th address location,
but also performs a $\enull$ check because $0$ is undefined in all heaps.
In addition to the no $\enull$ check in rule \rulelab{S-Def},
we perform checks in \rulelab{S-DefT} to ensure the existence of $\tmode$ mode pointers.
...
Predicate $n \not\in \Theta$ means that location $n$ is not freed in the $\tmode$ heap region,
while $n < |\heap(\tmode)|$ makes sure that $n$ does not go out of the high bound of the $\tmode$ heap region,
where $|\heap(\tmode)|$ refers to the cardinality of the $\tmode$ heap region.
If dynamic checks are violated, rules \rulelab{S-DefNull} and \rulelab{S-DefBound} are performed, meaning that the evaluation is transitioned into a runtime error that is correctly captured by the \systemname compiler.
...
\item $e=\eassign{\evalue{n}{\tptr{\tau}{\tmode}}}{\evalue{n'}{\tptr{\tau}{m'}}}$,
and $\estar{\evalue{n'}{\tptr{\tau}{m'}}}$ can make a move.

\item $e=\eassign{\evalue{n}{\tptr{\tau}{m}}}{\evalue{n'}{\tptr{\tau}{\tmode}}}$,
and $\estar{\evalue{n}{\tptr{\tau}{m}}}$ can make a move.
