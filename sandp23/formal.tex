...
as well as \lang’s meta-theories, including the type soundness, non-exposure, and non-crashing theorems.
...
It prevents unsafe types from being constructed.
  ...
  This is \emph{illegal} in~\lang{} because it potentially exposes the checked pointer addresses in a $\umode$ region;
  thus, it is rejected by the well-formedness. 
...
is formalized as 
{\footnotesize$\edyncast{\tptr{\tarrayb{n}{\tau}}{\cmode}}{e}$} in \lang. 
% 
...
any dynamic heap access of a tainted pointer requires a \textit{verification} ($\emptyset;\heap ; \emptyset \vdash_{\umode}\evalue{n_a}{\tau}$), which refers to that the pointer value $n_a$ is well-defined in $\heap(m,n_a)$ and has right type $\tau$.
...
where code in $\umode$ region cannot observe a valid checked pointer address.
