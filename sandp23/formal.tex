...
as well as \systemname’s meta-theories, including the type preservation, non-exposure, and clean separation theorems.
...
Every pointer type ($\tptr{\tau}{\xi}$) includes a
...
or unchecked (\umode), and a content type ($\tau$) denoting the valid type it is pointed to.
...
memory allocations ($\emalloc{\xi}{\omega}$) and free ($\efree{e}$), 
along with expressions that require special handling, such as, tainted blocks ($\eunchecked{\overline{x}}{e}$),
and checked blocks ($\echecked{\overline{x}}{e}$).
...
The heap allocation $\emalloc{\xi}{\omega}$ includes a mode flag $\xi$ for allocating memory in $\xi$ memory heap regions, i.e., heaps are separated into three different regions, classified by $\xi$.
The $\echeckedtext$ and $\euncheckedtext$ expressions are used to delimit \textit{code} regions,
where the former represents the $\cmode$ code region in a program that are trusted by users, and the latter represents non-trust $\umode$ code region.
To guarantee the clean separation property, we extend the C syntax to include $\echecked{\overline{x}}{e}$ and $\eunchecked{\overline{x}}{e}$ blocks,
where $\overline{x}$ represents all free variables that are allowed to communicate between instructions outside and inside of the block $e$, and it cannot contain checked pointers.
...
It prevents unsafe types from being constructed.
  Consider the type ~\code{_t_ptr<_Ptr<int>>}, which describes a tainted pointer of a checked pointer.
  This is not well-formed in~\lang{} because it potentially exposes the checked pointer addresses in a $\umode$ region when the unchecked ($\umode$) pointer is used. 
  Nevertheless, we can have a checked pointer whose element is a tainted pointers\mzs{,}\mzr{:}~\eg~\code{_ptr<_t_Ptr<int>>} is a valid type.
...
&& \mid \eif{E}{e}{e} \mid \eunchecked{\overline{x}}{E} \mid \echecked{\overline{x}}{E}
...
$\textcolor{blue}{\text{Dereference Operations:}}\\$
  \begin{mathpar}
    \inferrule[T-Def]
              {\xi \leq m \\\Gamma \vdash_m e : \tptr{\tau}{\xi}}
              {\Gamma \vdash_m \estar{e} : \tau}

        \inferrule[S-Def]{\xi \neq \tmode \\ \heap(n)=\evalue{n_a}{\tptr{\tau_a}{\xi}} }
    {(\varphi,\Theta,\heap,\estar{\evalue{n}{\tptr{\tau}{\xi}}}) \longrightarrow (\varphi,\Theta,\heap,\evalue{n_a}{\tau})}
    ...
    \inferrule[S-DefNull]{}{(\varphi,\Theta,\heap,\estar{\evalue{0}{\tptr{\tau}{\xi}}}) \longrightarrow (\varphi,\Theta,\heap,\enull)}
\qquad
    \inferrule[S-DefBoundA]{n \in \Theta}{(\varphi,\Theta,\heap,\estar{\evalue{n}{\tptr{\tau}{\tmode}}}) \longrightarrow (\varphi,\Theta,\heap,\ebounds)}
\qquad
    \inferrule[S-DefBoundB]{\heap(n)=\evalue{n_a}{\tptr{\tau_a}{\xi}}\\\xi\neq \xi'}{(\varphi,\Theta,\heap,\estar{\evalue{n}{\tptr{\tau}{\xi'}}}) \longrightarrow (\varphi,\Theta,\heap,\ebounds)}
  \end{mathpar}
...
$\textcolor{blue}{\text{Malloc and Free:}}\\$
  \begin{mathpar}
    \inferrule[T-Malloc]
              {\xi \leq m}
              {\Gamma \vdash_m \emalloc{\xi}{\tau} : \tptr{\tau}{\xi}}

    \inferrule[T-Free]
              {\xi \leq m \\\Gamma \vdash_m e : \tptr{\tau}{\xi}}
              {\Gamma \vdash_m \efree{e} : \tint}

  \inferrule[S-Malloc]{\mathtt{alloc}(\heap,\Theta,\xi,\tau)=(n,\heap')}
   { (\varphi,\Theta,\heap,\emalloc{\xi}{\tau}) \longrightarrow (\varphi,\Theta',\heap',\evalue{n}{\tptr{\tau}{\xi}})}

  \inferrule[S-Free]{\xi \neq \tmode}
   { (\varphi,\Theta,\heap,\efree{\evalue{n}{\tptr{\tau}{\xi}}}) \longrightarrow (\varphi,\Theta,\heap,\evalue{0}{\tint})}

  \inferrule[S-FreeT]{}
   { (\varphi,\Theta,\heap,\efree{\evalue{n}{\tptr{\tau}{\tmode}}}) \longrightarrow (\varphi,\{n\}\cup\Theta,\heap,\evalue{0}{\tint})}

  \end{mathpar}

$\textcolor{blue}{\text{Checked and Unchecked Context Switches:}}\\$
  \begin{mathpar}
    \inferrule[T-Checked]
              {\forall x\in\overline{x}\;.\;\neg\cmode(\Gamma(x))\\\neg\cmode(\tau)
                     \\\\\fv(e)\subseteq\overline{x}\\\Gamma;\Theta \vdash_c e : \tau}
              {\Gamma;\Theta \vdash_m \echecked{\overline{x}}{e} : \tau}

    \inferrule[T-Unchecked]
              {\forall x\in\overline{x}\;.\;\neg\cmode(\Gamma(x))\\\neg\cmode(\tau)
                \\\\ \fv(e)\subseteq\overline{x}\\\Gamma;\Theta \vdash_u e : \tau}
...
$\Theta$ is a set of $\tmode$ mode memory addresses that are freed at the point of executing $e$
and $\heap$ is a \emph{heap} mapping addresses (integer literals) to
values $\evalue{n}{\ptr{\tau}{\xi}}$; for both we ensure
$\fv(\tau)=\emptyset$.
In a heap value $\evalue{n}{\ptr{\tau}{\xi}}$,
$\tau$ represents the value's content type, while $\xi$ indicates the mode of the heap location;
we classify a heap location into $\cmode$, $\tmode$, and $\umode$ modes, corresponding to the three kinds of pointer modes.
...
As mentioned, value $\evalue{0}{\tptr{\tau}{\xi}}$
...
If the context hole locates in an $\euncheckedtext$, i.e., $E[\eunchecked{\Box}]$, the context region mode is $\umode$,
while if the hole locates in a $\echeckedtext$, i.e., $E[\echecked{\Box}]$, its context mode is $\cmode$.
...
Rules \rulelab{S-DefT} and \rulelab{S-Def} are for $\tmode$ mode and other mode pointer dereferences, respectively.
...
The pointer type ($\evalue{n}{\tptr{\tau}{\tmode}}$) in a dereference operation must have the same mode
as the one labeled in the type in the heap value $(\evalue{n_a}{\tptr{\tau_a}{\xi}}$).
The heap value type $\tptr{\tau_a}{\xi}$ is not an additional structure existed in the heap structure,
...
we perform checks in \rulelab{S-DefT} $n \not\in \Theta$ to ensure that location $n$ is not freed in the $\tmode$ heap region.
...
Rules $\rulelab{T-Malloc}$ and $\rulelab{T-Free}$ make sure that $\cmode$ and $\umode$ mode pointers are allocated and freed only in the $\cmode$ and $\umode$ regions, respectively, while $\tmode$ mode pointers are allowed to allocate and free in both regions.
The $\mathtt{alloc}(\heap,\Theta,\xi,\tau)$ function in rule \rulelab{S-Malloc} allocates a new pointer in the $\xi$ heap region.
If $\xi$ is $\tmode$ and $\Theta$ is not empty, the function picks a memory space in $\Theta$ as the new pointer;
otherwise, the function picks the $|\heap(\xi)|$ as the new pointer and increments $\heap(\xi)$'s cardinality.
The new location is labeled with a type annotation $\tptr{\tau}{\xi}$, indicating that the location is $\xi$ mode.
Every time when a deference and assignment is applied on a $\tmode$ pointer, the \systemname compiler applies a dynamic check to verify if the location is labeled with $\tmode$.
We can think of the dynamic check as $n \in \heap(\tmode)$, checking if a location $n$ is in a set $\heap(\tmode)$ containing all the $\tmode$ addresses. When a $\texttt{malloc}$ function is called to create a new $\tmode$ pointer, we expand set $\heap(\tmode)$ to include the new address. See \Cref{sec:compilation}.
Rules \rulelab{S-FreeT} and \rulelab{S-Free} define the \texttt{free} operation for $\tmode$ mode and other mode pointers, respectively.
Rule \rulelab{S-Free} "forgets" about the freed pointer, while \rulelab{S-FreeT} puts the freed $\tmode$ mode pointer address in the $\Theta$ set.
...
In this context switching, to guarantee the checked ($\cmode$) pointer non-exposure property, i.e.,
...
In \lang, we actually permits the accesses of checked pointers inside \code{StringAuth},
...
\item $e=\estar{\evalue{n}{\tptr{\tau}{\tmode}}}$.

\item $e=\eassign{\evalue{n}{\tptr{\tau}{\tmode}}}{\evalue{n'}{\tau'}}$.
