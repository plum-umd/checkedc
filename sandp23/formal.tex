...
Type and semantic rules for array types and pointer assignments are given in \Cref{appx:add-type-sem}.
...
The rule \textsc{T-Fun} (\Cref{fig:type-system-1}) states the type judgement for
...
substitution to every type \(\overline{\tau}\).
% 
...
\(\tau''\).
% 
\mz{I rewrote the sentences above.}
% 
Consider the \code{process_req2} function in
Fig.~\ref{lst:final}; its parameter type for \code{msg} 
depends on \code{m_1}. The \textsc{T-Fun} rule will
\mzu{substitute \code{n} with the argument}
\mz{\code{n} seems unbound here.}
at a call-site.
\ignore{
...
Type soundness relies on several \emph{well-formedness} given in \cite{li22checkedc} and \Cref{appx:add-type-sem}.
...
$\heap(\cmode)$ refers to all $\cmode$ mode pointers in heap $\heap$.
