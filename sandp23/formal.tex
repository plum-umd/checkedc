...
The~\fig{fig:checkc-syn} shows the syntax of~\lang.

\myparagraph{Type Syntax}
...
However, we can have a checked array whose elements are tainted pointers\mzs{,}\mzr{:}~\eg~\code{_Array_ptr<_t_Ptr<int>>} is a valid type.
% 
...
\end{array}
\]
The function type also has well-formed requirements (\fig{fig:wffunctions} in Appendix), which disallows nesting checked pointers inside tainted pointers.
...
The \lang system is a flow-sensitive, gradual type one that generates additional dynamic checks that are inserted in the typing checking stage and executed in the semantic evaluation stage.
...
\longrightarrow_m (\varphi',\heap',r)$, as partly shown in \Cref{fig:type-system-1} (labeled as \rulelab{S-}$X$).
 Here, $\varphi$ is a
...
We define a three point lattice $\xi_1 \le \xi_2$ to describe such permission, where $\tmode \le \xi$ and $m \le m$.
...
Pointer modes are also useful in determining if a nested pointer has a valid type.
For example, in a nested pointer $\tptr{(... \tptr{\tau}{\xi_2} ...)}{\xi_1}$, we require $\xi_2\le \xi_1$ to maintain non-exposure.

\lang also has static casting operations. As described in rule \textsc{T-CastPtr},
an pointer typed expression of type $\tptr{\tau_1}{\xi_1}$ can be casted to another pointer type \tptr{\tau_2}{\xi_2},
if \tptr{\tau_1}{\xi_1} subtypes ($\sqsubseteq_{\Theta}$) to \tptr{\tau_2}{\xi_2} $\tptr{\tau_1}{\xi_1} \sqsubseteq \tptr{\tau_2}{\xi_2}$.
...
//_Ptr<int> a; _t_Ptr<int> b; int *c;
c = (int *)b; // This is okay.
...
The complete subtyping relation was described in \Cref{fig:checkc-subtype}.
\ignore{
For example, the rule ($0\le b_l \wedge b_h \le 1 \Rightarrow \tptr{\tau}{\xi}\sqsubseteq
\tarrayptr{b_l}{b_h}{\tau}{\xi}$) permits treating a singleton
