...
Mainly, in a nested pointer $\tptr{(... \tptr{\tau}{\xi'} ...)}{\xi}$, $\xi'\le \xi$.
...
which permits the nested switching between $\echeckedtext$ (represented by context mode $\cmode$)
and $\euncheckedtext$ (represented by context mode $\umode$) code regions.
One example usage of the nested switching is the checked function callbacks inside 
...
A $\cmode$pointer is mapped to a heap location in the $\cmode$ region, 
while a $\tmode$ and $\umode$ pointer represents a $\umode$ region location.
We wrote $\heap(m,n)$ to retrieve the $n$-location heap value in the $m$ region,
and $\heapup{m}{n}{\evalue{n'}{\tau}}$ 
to update location $n$ with the value $\evalue{n'}{\tau}$ in the $m$ region.
...
The first five define deference and assignment operations---illustrate how the semantics checks bounds.
...
means we verify that the value $n_a$ is well-defined in $\heap(m,n_a)$ and has type $\tau$, if $\tau$ is a pointer.
...
and remember $x$ and its the new value $\evalue{n}{\tau}$
...
%
Rules \textsc{T-Def} and \textsc{T-AssignArr} check array
...
can only be deferenced in checked and unchecked regions, resp.;
...
$\sqsubseteq$ is given in Fig.~\ref{fig:checkc-subtype} and it is built on the type equality
...
There is another casting rule in \Cref{app:main} states that
...
$\cmode(\tau)$ judges a constant pointer 
...
we must substitute away this variable once it goes out of scope (\textsc{T-LetInt}). 
...
A type environment $\Gamma$ is well-formed iff every variable mentioned as type bounds in $\Gamma$ are bounded by $\tint$ typed variables in $\Gamma$.
\end{defi}
